const getMembersBoardById = (id, userId) => {
   ...
   return $.ajax({
      type: 'GET',
      url: `http://adrputra.somee.com/api/memberboards/board/${id}`,
...
const kickMember = (id) => {
   swal({
      ...
      if (willDelete) {
         $.ajax({
            type: 'DELETE',
            url: `http://adrputra.somee.com/api/memberboards/delete/${id}`,
            success: function (response) {
               location.reload()
            },
         })
...
const updateMember = () => {
   ...
   $.ajax({
      type: 'PUT',
      url: `http://adrputra.somee.com/api/memberboards`,
      data: JSON.stringify({
         id,
         userId: userId,
         boardId: boardId,
